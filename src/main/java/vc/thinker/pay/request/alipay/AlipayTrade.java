...
import vc.thinker.pay.request.utils.SignUtil;
...
public class AlipayTrade {
    ...
    public String TradeWapPayRequest(Map<String, String> sParaTemp) {
        ...
        String form = "";
        try {
            form = AliPayConfig.getAlipayClientInstance().pageExecute(alipayRequest).getBody();
     ...
     * url https://docs.open.alipay.com/203/107090
     *
     * @return 手机网页支付下单并支付
     */
    public String mobilePayRequest(Map<String, String> sParaTemp) {
        ...
        //自定义回传参数
        model.setPassbackParams(sParaTemp.get("passback_params"));
        ...
        try {
            ...
            logger.error("支付宝构造表单失败", e.getErrMsg());
        }
        logger.debug("支付宝支付表单构造:" + form);
        return form;
    }
    ...
    public AlipayTradeRefundResponse tradeRefundRequest(Map<String, String> sParaTemp) throws AlipayApiException {
        ...
        AlipayTradeRefundResponse response = AliPayConfig.getAlipayClientInstance().execute(request);
    ...
    public AlipayTradeFastpayRefundQueryResponse tradeRefundQuery(Map<String, String> sParaTemp) throws AlipayApiException {
        ...
        request.setBizContent(new Gson().toJson(sParaTemp));
        AlipayTradeFastpayRefundQueryResponse response = AliPayConfig.getAlipayClientInstance().execute(request);
        return response;
    }
    ...
    public AlipayTradeQueryResponse tradeOrderQuery(Map<String, String> sParaTemp) throws AlipayApiException {
        ...
        request.setBizContent(new Gson().toJson(sParaTemp));
        AlipayTradeQueryResponse response = AliPayConfig.getAlipayClientInstance().execute(request);
        return response;
    }
    ...
    public boolean verifyNotify(Map<String, String> paranMap) throws AlipayApiException {
        ...
        //调用SDK验证签名
        boolean isVerify = AlipaySignature.rsaCheckV1(paranMap, AliPayConfig.getInstance().getALIPAY_PUBLIC_KEY(), AliPayConfig.CHARSET);
