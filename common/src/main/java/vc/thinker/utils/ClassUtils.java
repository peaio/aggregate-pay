...
public abstract class ClassUtils {
	...
	public static Set<Class<?>> getClasses(String pack,Class<? extends Annotation> annotation) {
		...
		for (String p : packs) {
			beanDefinitions.addAll(provider.findCandidateComponents(p));
		}
		...
		for(BeanDefinition beanDefinition : beanDefinitions) {
			try {
				result.add(Thread.currentThread().getContextClassLoader().loadClass(beanDefinition.getBeanClassName()));
